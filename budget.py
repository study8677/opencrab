...
# 2026-05-26 校准:抽查近 15 次真实自改,文件数 max=3(中位 2)、行数区间 16–216
# (中位 48,仅 1 次 ≥110)。原 files=8 / lines=400 的停手线远高于每一次实际改动,等于
# 一道永不触发的边界——只看着像在守界,其实从不拦人。按观测包络收紧:文件留 5(给偶发
# 多文件留余量,6+ 即该警觉)、行数留 240(容得下那次最大的 216 合法样例,真超过则八成
# 是收不住了)。停手线要贴着真实行为,才拦得住「拿改动量伪装进步」的古德哈特滑坡。
DEFAULTS = {
    "minutes": 30,   # 一次自改的合理窗口;超了多半是卡住了,该收尾复盘
    "cmds": 20,      # 「动手」命令条数(由 --spend 记账,见下)
    "files": 5,      # 改动文件数;一次碰太多文件,回滚与审阅都变难(近 15 次 max=3)
    "lines": 240,    # 改动行数(增+删);大改应先在分支上养(见 branchlab.py;近 15 次 max=216)
