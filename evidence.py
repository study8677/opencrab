...
CLAIMS: list[Claim] = [
    ...
    Claim(
        name="patchcourse",
        # 自生手补丁课程表:把已验收的小修(补冒号/括号print/名字纠偏)从活招式库+赛题里抽成「招式骨架」
        # (认报错→在哪下手→怎么改一处→过哪道闸),并 replay 验一例——把样例坏源码丢回真 brain 管子
        # (brain_repair + 这门课的 oracle)跑一遍,确认如今仍修得通且真修对(不只是能启动)。复跑命令验证
        # 3 门已验收小修都抽得出、骨架齐全,且全表回炉通过——与 handsdojo 互补:一个把失败练成下次会,
        # 一个把成功钉成肌肉。
        asserts="3 类已验收小修能从活招式库抽成招式骨架,且 replay 回炉确认如今仍修得通、真修对(非仅能启动)",
        argv=_PY + ["patchcourse.py", "--selfcheck", "--quiet"],
        ttl_days=7,
        risk=2.0,
    ),
    Claim(
    ...
    Claim(
        name="tiergate",
        # 自生手升阶闸:把 brain-only 可接的活由易到难排成阶梯(语法真伤→纯函数小修→CLI 输出兼容改),
        # 升阶闸凭真实证据(下层 evidence 是否新鲜)判此刻资格上限——某级解锁⟺它及以下每级证据都新鲜,
        # 任一低级证据失守则资格当场跌回坎下。复跑命令验证阶梯成形、资格随证据单调升降、且一例 T3
        # 「CLI 输出兼容改」真补丁过 compat schema 判据(只新增键/无删键改型)+ 真过试衣间、破坏改被当场拒——
        # 会拒坏补丁还不够,得知道自己何时能安全接更难的活。
        asserts="brain 可接的活排成阶梯,升阶闸凭真实证据判此刻资格上限,且一例 T3 兼容改能过 schema 判据+试衣间、破坏改被拒",
        argv=_PY + ["tiergate.py", "--selfcheck", "--quiet"],
        ttl_days=7,
        risk=2.0,
    ),
    Claim(
    ...
    Claim(
        name="touchgraph",
        # 多文件触点闸:跨边界动手前先把这批要碰的文件之间的 import 触点采成一张有向带权图,
        # 凭图判这一爪跨不跨得起——文件数闸(一爪 ≤2 文件,否则触面太散)、耦合闸(双向依赖成环=最危险的缠拒、
        # 单向触点过深拒),证到「≤2 文件且低耦合」才放行;读不出/.py 解析坏一律保守拒收(非弃权),
        # 默认收紧。纯静态只 ast.parse 绝不执行。复跑命令验证单文件/低耦合两文件放行、深耦合/环/碰三个文件/
        # 文件不存在/语法坏各被拒在该拦的闸上、去重不误判、build_graph 邻居如实且永不抛错——
        # 单点爪已会稳落,下一步要学会安全地跨边界,而非蒙眼漫到第二个文件。
        asserts="跨文件小修动手前先画依赖触点图,只放行 ≤2 文件且低耦合的原子补丁,深耦合/环/触面太散/读不出一律保守拒收",
        argv=_PY + ["touchgraph.py", "--selfcheck", "--quiet"],
        ttl_days=7,
        risk=2.0,
    ),
]
